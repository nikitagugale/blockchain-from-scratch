...
impl StateMachine for WeirdSwitchMachine {
    ...
    fn next_state(starting_state: &TwoSwitches, t: &Toggle) -> TwoSwitches {
        // todo!("Exercise 2")
        match t {
            Toggle::FirstSwitch => if starting_state.first_switch {
                // Toggle the first switch off and turn off the second switch
                TwoSwitches {
                    first_switch: false,
                    second_switch: false,
                }
        ...
        ,
        Toggle::SecondSwitch => {
            // Toggle the second switch without affecting the first switch
            TwoSwitches {
